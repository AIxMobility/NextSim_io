/**
 * NextSim Captain
 * @file : FootpathRouteGenerator.cpp
 * @version : 1.0
 * @author : Yeonwoo Yu
 */
...
FootpathRouteGenerator::FootpathRouteGenerator() {}

std::atomic<long long> FootpathRouteGenerator::s_tempNodeCounter(0);

void FootpathRouteGenerator::LoadFootpathNetwork() {
    TiXmlDocument doc;
    bool loadSuccess = doc.LoadFile(NextSimIO::FootpathNetworkXMLPath.string().c_str());

    if (!loadSuccess) {
        std::cerr << "Error: Loading failed (FootpathRouteGenerator). Could not open or parse file: "
...
NearestLinkPoint FootpathRouteGenerator::FindNearestFootpathLinkPoint(double x, double y) const {
...
void FootpathRouteGenerator::AddFootpathNode(const FootpathNode& node) {
    m_footpathNodes[node.id] = node;
    // std::cout << "Added temporary node: " << node.id << std::endl; // For debugging
}

void FootpathRouteGenerator::AddFootpathEdge(const FootpathEdge& edge) {
...
const FootpathNode& FootpathRouteGenerator::GetFootpathNode(const std::string& nodeId) const {
...
void FootpathRouteGenerator::RemoveTempNodesAndEdges() {
...
double Captain::FootpathRouteGenerator::calculateDistance(const FootpathNode& n1, const FootpathNode& n2) const {
    return std::sqrt(std::pow(n1.x_coord - n2.x_coord, 2) + std::pow(n1.y_coord - n2.y_coord, 2));
}

double FootpathRouteGenerator::Dijkstra(const std::string& start_node_id, const std::string& end_node_id) {
...
double FootpathRouteGenerator::GetDistance(const std::pair<double, double>& origin_coords, const std::pair<double, double>& dest_coords) {
...
void FootpathRouteGenerator::ExportNetworkToCsv(const std::string& node_filepath, const std::string& edge_filepath, const std::string& path_filepath) const {
    // Save nodes to CSV (footpath_nodes.csv)
    std::ofstream nodes_ofs(node_filepath);
    if (!nodes_ofs.is_open()) {
        std::cerr << "[FootpathRouteGenerator::ExportNetworkToCsv][Error] Failed to open node file for writing: " << node_filepath << std::endl;
        return;
    }
    nodes_ofs << "id,x,y\n";
    ...
    nodes_ofs.close();
    std::cout << "\n[FootpathRouteGenerator::ExportNetworkToCsv][Info] Exported " << m_footpathNodes.size() << " nodes to " << node_filepath << std::endl;

    // Save edges to CSV (footpath_edges.csv)
    std::ofstream edges_ofs(edge_filepath);
    if (!edges_ofs.is_open()) {
        std::cerr << "[FootpathRouteGenerator::ExportNetworkToCsv][Error] Failed to open edge file for writing: " << edge_filepath << std::endl;
        return;
    }
    ...
    edges_ofs.close();
    std::cout << "[FootpathRouteGenerator::ExportNetworkToCsv][Info] Exported " << m_footpathEdges.size() << " edge lists to " << edge_filepath << std::endl;

    // Save paths to CSV (footpath_paths.csv)
    std::ofstream paths_ofs(path_filepath);
    if (!paths_ofs.is_open()) {
        std::cerr << "[FootpathRouteGenerator::ExportNetworkToCsv][Error] Failed to open path file for writing: " << path_filepath << std::endl;
        return;
    }
    paths_ofs << "node_id\n";
    ...
    paths_ofs.close();
    std::cout << "[FootpathRouteGenerator::ExportNetworkToCsv][Info] Exported " << m_footpath.size() << " path nodes to " << path_filepath << std::endl;
}
